...
<body>
   ...
   $id = $_SESSION['id'];
    ...
    if (count($rs) >= 1) {
        foreach ($rs as $v) {
           if ($v['fromC']==$id) {
            echo "<div class='text-end'  style='background-color:green;margin:5px;border-radius:7px;color:white;'>{$v['conten']}/<spane style='font-size: 10px;'>{$v['dateC']}</sapne></div>";
           }
           else{
            echo "<div class='text-start' style='background-color:#00bfff;margin:5px;border-radius:7px;color:white;'>{$v['conten']}/<spane style='font-size: 10px;'>{$v['dateC']}</sapne></div>";
           }
...
if (isset($_POST['sub'])) {
    ...
    $req = $connexion->prepare("INSERT INTO `chat`(`fromC`, `toC`, `conten`,`dateC`) VALUES ('$id','admin','$message','$currentDateTime')");
    $req->execute();
    echo "<script>window.location='roomP.php';</script>";
    exit; 
}
