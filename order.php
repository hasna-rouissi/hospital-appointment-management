                    ...
                    <div class="bg-white rounded p-5 m-5 mb-0">
                        <form action="" method="post">
                             <div class="row g-3">
                              ...
                              <div class="col-12">
                                    <input type="text" name='code' class="form-control bg-light border-0" placeholder="Enter verification code" style="height: 55px;">
                                </div>
                             
                                <div class="col-12">
                                <input type='submit' name='sub' class="btn btn-primary w-100 py-3" value="Check">
                                </div>
                                </form>
                                <?php
                               if (isset($_POST['sub'])) {
                                $code=$_POST['code'];
                                $req=$connexion->prepare("select code from accepted where code='$code'");
                                $req->execute();
                                $res=$req->fetchAll();
                                if (count($res) >= 1) {?>
                                <h3 style='color:green;' class='text-center'>Accepted</h3>
                                
                                <?php
                                   
                                }
                                else{?>
                                <h3 style='color:red;' class='text-center'>Not Accepted</h3>
